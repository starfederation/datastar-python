"""Integration test: datastar_response within a live FastHTML app."""
...
@pytest.mark.anyio
async def test_fasthtml_sync_and_streaming_handlers() -> None:
    """Ensure FastHTML routes across sync/async and gen/value work end-to-end."""

    entered = threading.Event()

    app, rt = fast_app(htmx=False, live=False)

    @rt("/slow")
    @datastar_response
    def slow(request) -> Any:  # noqa: ANN001
        entered.set()
        time.sleep(0.2)  # should not block event loop for other requests
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=base_url) as client:
            async with anyio.create_task_group() as tg:
                ...
                async def hit_slow():
                    nonlocal slow_resp
                    slow_resp = await _fetch(client, "/slow")
            ...
            assert float(ping_resp.elapsed.total_seconds()) < 0.3
            ...
            stream = await _fetch(client, "/stream")
            body = stream.text
            assert '"src":"async_generator"' in body
            assert '"idx":1' in body and '"idx":2' in body
    finally:
